...
def find_pnr_id(name):
    cmd = f"select pnr_id from passengers where name = '{name}'"
    cursor.execute(cmd)
    out = cursor.fetchone()[0]
    return out

def checkin(pnr_id):
    cmd = f"select * from reservations where g_id = '{pnr_id}';"
    cursor.execute(cmd)
    reservation = cursor.fetchall()
    if reservation != []:
        subcmd = f"update reservations set check_in = curdate() where g_id = '{pnr_id}' "
...
# Get all passengers
def get_passengers():
    cmd = "select pnr_id, name, address, email_id, phone, created_at from passengers;"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
...
# Add a passenger
def add_passenger(name, address, email_id, phone):
    cmd = f"insert into passengers(name,address,email_id,phone) values('{name}','{address}','{email_id}',{phone});"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
# add a lounge
def add_lounge(lounge_no, price, lounge_type):
    cmd = f"insert into loungeArea(lounge_no,price,lounge_type) values('{lounge_no}',{price},'{lounge_type}');"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
# Get All lounge
def get_lounges():
    cmd = "select id, lounge_no, lounge_type, price, created_at from loungeArea;"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return cursor.fetchall()
...
# Get all room count
def get_total_lounges():
    cmd = "select count(lounge_no) from loungeArea;"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
...
def booked():
    cmd = f"select count(ros.id) from reservations rs, loungeArea ros where rs.r_id = ros.id and rs.check_out is Null;"
    cursor.execute(cmd)
...
def vacant():
    return get_total_lounges() - booked()
...
def get_total_Lounge_value():
    cmd = "select sum(price) from loungeArea;"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
...
def delete_lounge(id):
    cmd = f"delete from loungeArea where id='{id}';"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
def delete_passengers(id):
    cmd = f"delete from passengers where pnr_id='{id}';"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
def update_lounge(id, lounge_no, lounge_type, price):
    cmd = f"update loungeArea set lounge_type = '{lounge_type}',price= {price}, lounge_no = {lounge_no} where id = {id};"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
...
def update_passenger(name, address, pnr_id, phone):
    cmd = f"update passengers set address = '{address}',phone = {phone} , name = '{name}' where pnr_id = {pnr_id};"
    cursor.execute(cmd)
    if cursor.rowcount == 0:
        return False
    return True
